...
erDiagram
  ...
  projects ||--o{ project_notes : "project_id (cascade)"
...
| 🔒 revenue | numeric | default 0 | **Tổng doanh thu dự kiến** (triệu) — nuôi margin kế hoạch (D5) |
| 🔒 other_cost | numeric | default 0 | Chi phí khác ngoài lương (triệu) |
| 🔒 mgmt_pct | numeric | default 0 | % management overhead (D16) |
| 🔒 revenue_collected | numeric | default 0 | **Thực thu** — đã thu thực tế (triệu, D27). Dự thu còn lại = `max(0, revenue − revenue_collected)` (tự tính, không lưu). KHÔNG đụng margin. |
...
### 3.6 `project_notes` — Ghi chú dự án (free-text, mọi user CRUD)
| Cột | Kiểu | Ràng buộc | Ghi chú |
|---|---|---|---|
| id | uuid | PK | |
| project_id | uuid | FK → projects, **CASCADE** | Xóa dự án → xóa ghi chú |
| body | text | NOT NULL, default '' | Nội dung ghi chú |
| author | text | | Email người tạo (chỉ để hiển thị) |
| created_at | timestamptz | default now() | |
| updated_at | timestamptz | default now() | Tự cập nhật qua trigger `trg_project_notes_touch` mỗi lần UPDATE |

> Ghi chú dự án **KHÔNG nhạy cảm tài chính** → **mọi user (pm & finance)** đều tạo/sửa/xóa được (RLS `note_rw` = `using(true)`). Hiển thị ở card "📝 Ghi chú dự án" tab Chi tiết dự án (t3), lưu thẳng Supabase (độc lập nút Lưu dự án). **KHÔNG phải tracking tiến độ (D1)** — chỉ là sổ ghi chú kế hoạch/thông tin.
...
| `trg_guard_project_fin` → `guard_project_financials()` | `projects` (before INSERT/UPDATE) | Non-finance: INSERT ép `revenue/other_cost/mgmt_pct/revenue_collected = 0`; UPDATE giữ giá trị cũ → **PM không gài được số tiền** (gồm cả thực thu) |
| `trg_project_notes_touch` → `touch_updated_at()` | `project_notes` (before UPDATE) | Tự đặt `updated_at = now()` mỗi lần sửa ghi chú |
...
- **`project_notes`**: authenticated (pm & finance) **toàn quyền CRUD** (`note_rw using(true)`) — ghi chú dự án không nhạy cảm tài chính.
...
| Ghi chú dự án (`project_notes`) | đọc/ghi/xóa | đọc/ghi/xóa |
...
| CASCADE | xóa project → xóa phases/allocations/assignments/project_notes; xóa employee → xóa assignments |
