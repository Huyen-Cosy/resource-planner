...
- **t5 — Đóng dự án**: nhập actual theo role (prefill = estimate, chỉ sửa ô lệch), chênh lệch % tự tính, close_note. → ghi allocations kind='actual', status='closed'. *(⏭ 15/06 — D22: khi build đợt burn, t5 đổi vai thành **nghi thức đóng** (state + bài học + snapshot role); việc nhập **giờ effort thực** chuyển sang **t3** ở lớp burn-actual tầng người — xem DECISIONS D22–D24. Chưa build.)*
