...
- **Bước tiếp theo:** (0) **Deploy `web/` lên Vercel** (Root Directory = `web`, branch `main`) → lấy URL `*.vercel.app` → đăng ký URL vào **Supabase → Auth → Site URL** (qua Management API, cần lại token `sbp_…`). *Lưu ý: deploy bằng CLI từ Claude Code cần host `api.vercel.com`/`vercel.com` trong network allowlist (đang được thêm); nếu chưa có thì deploy tay trên dashboard.* (1) ẩn ô tiền cho `pm` (gắn `.fin-only` vào output render tài chính — CSS đã sẵn); (2) màn admin "Quản lý người dùng" (finance nâng pm→finance); (3) Phase 3 gán người nâng cao (t7) + Phase 4 CEO/đóng dự án (t0/t5 ghi actual). t4 Cách 2 (AI) + import CSV vào DB để Phase 5.
